	...
	public List<Object[]> findBycountGraficaAsignarInforme(String periodo) {
		...
		try {
			String sql = "SELECT"
					...
					+ "	(SELECT COUNT ( * ) FROM esq_retroalimentacion.tbl_asignacion asig2 where asig2.estado_asig ='asignado' and asig2.identificador = asig.identificador and asig2.periodo_asig =" + periodo + ") AS asginado,\r\n"
					+ "(SELECT COUNT ( * ) FROM esq_retroalimentacion.tbl_asignacion asig2 where asig2.estado_asig ='calificado' and asig2.identificador = asig.identificador and asig2.periodo_asig =" + periodo + ") AS calificado 	\r\n"
					+ "FROM\r\n" + "	esq_retroalimentacion.tbl_asignacion asig " + "WHERE\r\n"
					+ " asig.periodo_asig =" + periodo + " GROUP BY "
					+ "	asig.identificador";
			Query query = em.createNativeQuery(sql);
			lista = query.getResultList();
