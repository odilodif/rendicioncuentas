...
implements QuerysNativosRepository {
	...
	public List qryBuscarInformesNativo(String provincia, String periodo , String funcion , String descripcion) {
		...
		try {
			String sql="SELECT \r\n" + 
					...
					"	AND insti.inst_cod NOT IN ( SELECT inst_cod FROM esq_retroalimentacion.tbl_asignacion asig WHERE asig.estado_asig = 'asignado' OR asig.estado_asig = 'calificado')";
			Query query=em.createNativeQuery(sql);
			lista=query.getResultList();
	...
	public List qryContarInformesNativoSinDescripcion(String provincia, String periodo , String funcion ) {
		...
		try {
			String sql="SELECT COUNT \r\n" + 
					...
					"	and insti.prov_cod = "+provincia+" \r\n" + 
					"	and inst_periodo = "+periodo+" \r\n" + 
					"	AND insti.inst_cod NOT IN ( SELECT inst_cod FROM esq_retroalimentacion.tbl_asignacion asig" +
					"   WHERE asig.estado_asig = 'asignado' OR asig.estado_asig = 'calificado' )";
			Query query=em.createNativeQuery(sql);       
			 lista=query.getResultList();
		} catch (Exception e) {
			System.out.println(e.getMessage());
	...
	public List qryBuscarInformesAsigandosxtecnicoNativo(String tecnico, String periodo, String estadoAsig) {
		...
		try {
			String sql="SELECT\r\n" + 
					...
					"   LEFT JOIN esq_retroalimentacion.tbl_valoracion as valo on valo.insti_cod = asig.inst_cod \r\n" +
					"WHERE\r\n" + 
	...
	public List qryBuscarInformesValoradosSubCoord( String periodo, String estadoAsig) {
		...
		try {
			String sql="SELECT\r\n" + 
					...
					"					asig.estado_asig, \r\n" + 
					"					(SELECT case  WHEN val.nota >= 100 then 100 else CAST(FLOOR(val.nota*100)/100 as numeric(10,2)) END FROM esq_retroalimentacion.tbl_valoracion val WHERE val.inf_cod = asig.inf_cod)  as nota,\r\n" + 
					"					insti.inst_tf_cod\r\n" + 
					"FROM\r\n" + 
					"					esq_retroalimentacion.tbl_asignacion AS asig\r\n" + 
					"					INNER JOIN esq_catalogos.tbl_institucion AS insti ON asig.inst_cod = insti.inst_cod \r\n" + 
	...
	@SuppressWarnings({ "finally", "rawtypes", "unchecked" })
	@Override
	public List qryBuscarInformesValoradosxSubcoordinadorNativo(String periodo, String estadoAsig) {
		EntityManager em= emf.createEntityManager();
		List lista=null;
		try {
			String sql="SELECT\r\n" + 
					...
					"	insti.inst_tf_cod, \r\n" +
					"  ( SELECT funcio.nombre FROM esq_usuarios_internos.tbl_datos_funcionarios funcio WHERE funcio.identificador = asig.identificador ) AS asignado, \r\n"+
					"  ( SELECT valor.fecha_valoracion FROM esq_retroalimentacion.tbl_valoracion valor WHERE valor.inf_cod = asig.inf_cod ) AS fechaValo \r\n"+
					"FROM\r\n" + 
					"	esq_retroalimentacion.tbl_asignacion AS asig\r\n" + 
					"	INNER JOIN esq_catalogos.tbl_institucion AS insti ON asig.inst_cod = insti.inst_cod \r\n" + 
					"WHERE\r\n" + 
