...
public interface UsuarioInternoRepository extends JpaRepository<TblDatosFuncionarios, String> {
	...
	@Query("select distinct d from TblDatosFuncionarios d JOIN TblAsignacion a ON d.identificador = a.identificador where d.estadoEmpleado=:estadoEmpleado and d.cargo!=:cargo and a.periodoAsig =:periodo order by d.nombre asc")
	List<TblDatosFuncionarios> findByEstadoEmpleadoAndCargoTecnicoAndPeriodo(@Param("estadoEmpleado") String estadoEmpleado,
			@Param("cargo") String cargo, @Param("periodo")Integer periodo);
