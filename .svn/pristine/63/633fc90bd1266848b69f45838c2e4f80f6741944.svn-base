...
public interface InstitucionRepository extends JpaRepository<TblInstitucion, String> {
	...
	@Query("SELECT COUNT(*)  from TblInstitucion where instPeriodo=:instPeriodo and provCod=:provCod and instEstado <> 'eliminado'")
	...
	//Query para obtener los años en que la institucion presento un informe	
	@Query("SELECT t.instCod, t.instPeriodo FROM TblInstitucion t WHERE t.instEstado <> 'eliminado' AND t.instRuc =:ruc AND t.instVerificador IS NOT NULL ORDER BY t.instPeriodo ASC")
		List<TblInstitucion> findByRucAndVerificadorNotNull(@Param("ruc") String ruc);
	@Query("SELECT t.instCod, t.instPeriodo FROM TblInstitucion t WHERE t.instEstado <> 'eliminado' AND t.instRuc =:ruc AND t.instVerificador IS NOT NULL  AND provCod=:provincia AND cantCod =:canton ORDER BY t.instPeriodo ASC")
	List<TblInstitucion> findByRucAndVerificadorNotNullMedios( @Param("ruc") String ruc,  @Param("provincia") Integer provincia, @Param("canton")  Integer canton);
